...
function App() {
  ...
  const openView = (nextView: 'report' | 'setup') => {
    setView(nextView)
    if (nextView === 'setup') setSourceWindows([])
  }
      ...
      <header className="simple-header">
        ...
        <div className="header-center">
          <div className="document-title">
            <strong>Muster Verpackungen · Claim Compiler</strong>
      ...
      <main className={`workspace ${panelOpen ? 'panel-open' : ''}`}>
        <section className="document-workspace">
          <div className="document-status-bar">
            ...
            <span>27,190 rows loaded</span>
            <span>4 findings</span>
            <button type="button" onClick={() => notify('5 typed gates · 14 cited passages')}><Link2 size={13} /> 14 citations</button>
          </div>

          <ClaimCompiler onSource={openSourceWindow} onNotify={notify} />
  ...
  return (
    <section className="evidence-dock">
      <div className="dock-heading">
        ...
        <small>14 cited passages · click a source · drag to compare</small>
      </div>
  ...
  return (
    <div className="findings-panel">
      <div className="panel-intro">
        <ShieldCheck size={16} />
        <div><strong>4 findings</strong><span>14 cited passages · {citedFileCount} cited files</span></div>
      </div>
      {findings.map((finding) => {
          ...
          <article className={`finding-card ${expanded ? 'expanded' : ''}`} key={finding.id}>
            ...
            {expanded && (
              <div className="finding-body">
                ...
                <div className="evidence-row"><span><ShieldCheck size={12} /> {finding.sources.length} passages</span><button type="button" onClick={onReview}><Check size={13} /> Review</button></div>
