...
import { analyzeFiles, loadSample, runSpecialists } from './analysis'
...
export function SourceDataDialog({ currentName, onClose, onAnalysis }: { currentName: string; onClose: () => void; onAnalysis: (analysis: Analysis) => void }) {
  ...
  const sample = async () => {
    setBusy(true)
    setError('')
    setPhase('Loading sample dossier')
    try {
      const analysis = await loadSample()
      onAnalysis(analysis)
      onClose()
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Could not load sample')
    } finally {
      setBusy(false)
      setPhase('')
    }
  ...
  return <div className="source-dialog-backdrop" role="presentation">
    <section className="source-dialog" role="dialog" aria-modal="true" aria-labelledby="source-dialog-title">
      ...
      <footer><button type="button" onClick={() => void sample()} disabled={busy}><RotateCcw size={15} /> Sample</button><span /><button type="button" onClick={onClose} disabled={busy}>Cancel</button><button type="button" className="primary" onClick={() => void run()} disabled={busy || !files.length}>{busy ? <LoaderCircle size={15} /> : <FileUp size={15} />} Analyze</button></footer>
    </section>
  </div>
}
