...
import type { ManualCaseInput } from './AddCaseDialog'
...
function App() {
  ...
  const [appView, setAppView] = useState<'investigation' | 'setup'>('investigation')
  const setupHealth = useSetupHealth()

  const activeFinding = findings.find((finding) => finding.id === activeBundle) ?? findings[0]
  const serviceReady = {
    cognee: setupHealth.adapterChecks.cognee.phase === 'ok' || setupHealth.runtimeStatus?.adapters.cogneeReady === true,
    tavily: setupHealth.adapterChecks.tavily.phase === 'ok' || setupHealth.runtimeStatus?.adapters.tavilyReady === true,
    codex: setupHealth.adapterChecks.codex.phase === 'ok' || setupHealth.runtimeStatus?.adapters.codexSidecarReady === true,
  }
  ...
  const saveManualCase = (input: ManualCaseInput) => {
    const id = `M-${String(manualCases.length + 1).padStart(2, '0')}`
    ...
    setManualCases((items) => [manualCase, ...items])
    ...
    <div className="simple-app">
      <header className="simple-header">
        ...
        <nav className="app-navigation" aria-label="Primary">
          <button type="button" className={appView === 'investigation' ? 'active' : ''} onClick={() => setAppView('investigation')}>Investigation</button>
          <button type="button" className={appView === 'setup' ? 'active' : ''} onClick={() => setAppView('setup')}>Setup</button>
        </nav>
      ...
      <div className="editor-toolbar">
        ...
        <button type="button" className="mobile-findings-button" onClick={() => { setPanelOpen(true); setRightTab('findings') }}><ShieldCheck size={15} /> Review {findings.length + manualCases.length}</button>
      ...
      <main className="workspace">
        <section className="document-workspace">
          <div className="document-status-bar">
            ...
            {manualCases.length > 0 && <span>{manualCases.length} added</span>}
          ...
          <InvestigationPlanner
            ...
            onOpenSetup={() => setAppView('setup')}
            onAddCase={() => setCaseDialogOpen(true)}
            onNotice={notify}
          />
        ...
        <aside className={`review-panel ${panelOpen ? 'panel-open' : ''}`}>
          <div className="panel-tabs">
            <button type="button" className={rightTab === 'findings' ? 'active' : ''} onClick={() => setRightTab('findings')}>Review <span>{findings.length + manualCases.length}</span></button>
        ...
        <SetupPage controller={setupHealth} onBack={() => setAppView('investigation')} onNotice={notify} />
      )}
...
function FindingsPanel({ expandedFinding, manualCases, onExpanded, onSource, onAddCase, onReview }: FindingsPanelProps) {
  return (
    <div className="findings-panel">
      <div className="panel-intro">
        <ShieldCheck size={16} />
        <div><strong>Review queue</strong><span>4 test findings{manualCases.length > 0 ? ` · ${manualCases.length} added` : ''}</span></div>
      </div>
      <div className="queue-toolbar">
        <span>{findings.length + manualCases.length} items</span>
        <button type="button" onClick={onAddCase}>+ Add case</button>
      </div>
      {manualCases.map((manualCase) => {
        const expanded = manualCase.id === expandedFinding
        return (
          <article className={`finding-card manual-case-card ${expanded ? 'expanded' : ''}`} key={manualCase.id}>
            <button type="button" className="finding-summary" onClick={() => onExpanded(expanded ? '' : manualCase.id)}>
              <span className="risk-dot manual" />
              <span className="finding-summary-copy"><small>{manualCase.id} · {manualCase.category}</small><strong>{manualCase.title}</strong></span>
              <strong className="finding-amount">{manualCase.amount}</strong>
              {expanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
            </button>
            {expanded && (
              <div className="finding-body manual-case-body">
                {manualCase.notes && <p>{manualCase.notes}</p>}
                <div className="finding-meta-row"><span><Link2 size={12} /> {manualCase.sources.length} passages</span><small>Added now</small></div>
                <div className="source-label">SOURCES</div>
                <div className="source-list">
                  {manualCase.sources.map((source) => (
          ...
          <article className={`finding-card ${expanded ? 'expanded' : ''}`} key={finding.id}>
            ...
            {expanded && (
              <div className="finding-body">
                ...
                <div className="finding-meta-row"><span><ShieldCheck size={12} /> {finding.sources.length} passages</span><button type="button" onClick={onReview}><Check size={13} /> Review</button></div>
