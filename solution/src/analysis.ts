...
export async function loadFinal() {
  return readResponse(await fetch(`${ENGINE}/api/load-final`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: '{}',
  }))
...
export async function selectPreset(kind: 'first' | 'final') {
  return readResponse(await fetch(`${ENGINE}/api/select`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ kind, enabled: { cognee: true, tavily: true, codex: true } }),
  }))
}
